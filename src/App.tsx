...
// ID client OAuth Web (Google Identity Services) — public par nature (visible côté client).
const GOOGLE_CLIENT_ID = '107344112497-p0h1o9qr0edh796kmt1dv5gcnb8c4nhf.apps.googleusercontent.com';
...
function estimateRetailHtgCents(faceUsdCents: number, r: { discountBps: number; fixedFeeUsdCents: number; feeBps: number }): number {
  const ACQ = 14200, CRYPTO = 100, MARGIN = 1500, ROUND = 500;
  const wholesale = (faceUsdCents * (10000 - r.discountBps)) / 10000 + r.fixedFeeUsdCents + (faceUsdCents * r.feeBps) / 10000;
  const usdt = (wholesale * (10000 + CRYPTO)) / 10000;
  const costHtg = (usdt * ACQ) / 100;
  const retail = costHtg / ((10000 - MARGIN) / 10000);
  return Math.ceil(retail / ROUND) * ROUND;
}
...
export default function App() {
  ...
  // Logique post-connexion Google commune (popup OU Google Identity Services) : crée le doc user
  // au 1er login, ferme la modale et renvoie à l'accueil.
  const completeGoogleSignIn = async (currentUser: FirebaseUser) => {
    const userRef = doc(db, 'users', currentUser.uid);
  ...
  const reportGoogleError = (err: any) => {
    console.error(err);
    const errMsg = lang === 'FR'
      ? `Échec de l'authentification Google : ${err?.message ?? ''}`
      : `Echwe Google autentifikasyon : ${err?.message ?? ''}`;
    if (currentPage === 'login-screen') setLoginError(errMsg);
    else if (currentPage === 'register-screen') setRegisterError(errMsg);
    else alert(errMsg);
  };

  /** Flux Google Identity Services (bouton officiel) : échange l'ID token contre une session Firebase. */
  const handleGoogleCredential = async (idToken: string) => {
    setAuthLoading(true);
    setLoginError(null); setRegisterError(null);
    try {
      const result = await signInWithCredential(auth, GoogleAuthProvider.credential(idToken));
      await completeGoogleSignIn(result.user);
    } catch (err: any) {
      reportGoogleError(err);
    } finally {
      setAuthLoading(false);
    }
  };
          ...
          {currentPage === 'welcome' && (
            <div className="flex flex-col items-center text-center">
              ...
              <div className="w-full flex flex-col gap-3.5 mt-8">
                ...
                <div id="welcome-google-btn" className="w-full">
                  <GoogleSignInButton clientId={GOOGLE_CLIENT_ID} theme={theme} onCredential={handleGoogleCredential} />
                </div>
      ...
      {authModalOpen && (
        <div id="auth-modal" className="fixed inset-0 z-50 bg-black/80 overflow-y-auto animate-fadeIn backdrop-blur-sm">
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            {/* Séparateur + bouton OFFICIEL Google Identity Services (sélecteur de compte / One Tap) */}
            <div className="flex items-center my-4 select-none">
              <div className="flex-grow border-t border-[var(--tt-border)]"></div>
              <span className="px-4 text-[10px] font-mono text-[var(--tt-text-faint)] uppercase tracking-widest">{lang === 'FR' ? 'OU' : 'OSWA'}</span>
              <div className="flex-grow border-t border-[var(--tt-border)]"></div>
            </div>
            <GoogleSignInButton
