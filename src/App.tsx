      ...
      {authModalOpen && (
        <div id="auth-modal" className="fixed inset-0 z-50 bg-black/80 overflow-y-auto animate-fadeIn backdrop-blur-sm">
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            {/* Séparateur + connexion Google (sélecteur de compte) */}
            <div className="flex items-center my-4 select-none">
              <div className="flex-grow border-t border-[var(--tt-border)]"></div>
            ...
            <button
              ...
              onClick={handleGoogleSignIn}
              disabled={authLoading}
              className="w-full bg-[var(--tt-surface-2)] hover:bg-[var(--tt-surface-3)] border border-[var(--tt-border)] hover:border-[var(--tt-border-strong)] text-[var(--tt-text)] font-bold text-sm py-3.5 rounded-2xl flex items-center justify-center gap-2.5 transition-all disabled:opacity-50 cursor-pointer"
            >
              {authLoading ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
